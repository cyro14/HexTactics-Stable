...
const LEADERS = [
    ...
    { id: 'L_HARPY', name: 'Matriarca Harpia', loreFaction: 'SILVESTRE', emoji: '🦅', hp: 65, maxHp: 65, mp: 5, maxMp: 5, atk: 14, limit: 3, range: 1, tags: ['WING', 'STALKER'], fav: ['MOUNTAIN'], filter: 'saturate(150%)', abilities: ['flying', 'dodge'], desc: 'Senhora dos ventos. Ignora custos de terreno pesado (sempre move por 1 MP) e possui esquiva natural alta.', sprite: 'img/lideres/harpia.png' },
    ...
    { id: 'ld_gargula', name: 'Gárgula Anciã', loreFaction: 'FORASTEIROS', emoji: '🗿', maxHp: 200, hp: 200, mp: 3, maxMp: 3, atk: 35, range: 1, limit: 3, fav: 'MOUNTAIN', tags: ['ROCK', 'WING'], abilities: ['flying', 'counter'], desc: 'Estátua viva de altíssima defesa. Voa e pune quem a ataca de perto.', sprite: 'img/lideres/gargula.png' },
    { id: 'ld_golem_gelo', name: 'Golem de Gelo', loreFaction: 'FORASTEIROS', emoji: '🧊', maxHp: 250, hp: 250, mp: 3, maxMp: 3, atk: 30, range: 1, limit: 3, fav: 'SNOW', tags: ['ROCK', 'ICE'], abilities: [], desc: 'Um colosso glacial. Caminha devagar, mas congela os mares.', sprite: 'img/lideres/golem_gelo.png' },
...
const ITEMS = {
    ...
    'THUNDER_AMULET': { id: 'THUNDER_AMULET', name: 'Amuleto da Tormenta', icon: '⚡', desc: '+10% Crítico. Ataques zeram MP inimigo.', equipSlot: 'accessory', onEquip: (u, lvl) => { u.critChance = (u.critChance || 0) + 0.10; if (!u.abilities.includes('shock')) u.abilities.push('shock'); }, onUnequip: (u, lvl) => { u.critChance -= 0.10; let idx = u.abilities.indexOf('shock'); if (idx > -1) u.abilities.splice(idx, 1); } }, 'FROST_ARMOR': { id: 'FROST_ARMOR', name: 'Manto do Zero Absoluto', icon: '❄️', desc: '+30 HP. Aura Congelante.', onEquip: (u, lvl) => { u.maxHp += 30 * lvl; u.hp += 30 * lvl; if (!u.abilities.includes('frost_aura')) u.abilities.push('frost_aura'); }, onUnequip: (u, lvl) => { u.maxHp -= 30 * lvl; u.hp = Math.min(u.hp, u.maxHp); let idx = u.abilities.indexOf('frost_aura'); if (idx > -1) u.abilities.splice(idx, 1); } },
    'HELLFIRE_BOW': { id: 'HELLFIRE_BOW', name: 'Arco do Inferno', icon: '🏹🔥', desc: '+1 Alcance, +12 ATK. Incendeia alvos.', equipSlot: 'weapon', onEquip: (u, lvl) => { u.range = (u.range || 1) + 1; u.atk += 12 * lvl; if (!u.abilities.includes('burn')) u.abilities.push('burn'); }, onUnequip: (u, lvl) => { u.range -= 1; u.atk -= 12 * lvl; let idx = u.abilities.indexOf('burn'); if (idx > -1) u.abilities.splice(idx, 1); } }, 'WINGED_SHIELD': { id: 'WINGED_SHIELD', name: 'Escudo Alado', icon: '🛡️', desc: '+15 HP, +25% Esquiva.', onEquip: (u, lvl) => { u.maxHp += 15 * lvl; u.hp += 15 * lvl; if (!u.abilities.includes('dodge')) u.abilities.push('dodge'); }, onUnequip: (u, lvl) => { u.maxHp -= 15 * lvl; u.hp = Math.min(u.hp, u.maxHp); } }
};
